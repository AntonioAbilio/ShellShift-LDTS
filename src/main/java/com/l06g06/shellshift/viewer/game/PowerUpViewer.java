...
public class PowerUpViewer implements ElementViewer<PowerUp> {
    ...
    public void draw(PowerUp element, Gui gui) {
        if (element instanceof SpeedPowerUp)
            gui.drawImageASCII(Components.SpeedPowerUp.getImage(), element.getPosition());
        if (element instanceof StarPowerUp)
            gui.drawImageASCII(Components.StarPowerUp.getImage(), element.getPosition());
        if (element instanceof BulletPowerUp)
            gui.drawImageASCII(Components.BulletPowerUp.getImage(), element.getPosition());
    }
}
